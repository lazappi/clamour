...
tweets %>% 
    ...
    ggplot(aes(reorder(source, n), n)) +
        ...
        labs(x = "Source", y = "Users",
             title = paste(params$hashtag, "users by source"),
...
tweets %>% 
    ...
    count(text) %>%
    filter(
        text != str_remove(params$hashtag, "#"), 
        n >= 5
    ) %>%
    ggplot(aes(reorder(text, n), n)) +
        ...
        labs(x = "Hashtag", y = "Tweets",
             title = paste("Other hashtags used with", params$hashtag),
             subtitle = "alternative hashtags used at least 5 times")
```

## Emojis

```{r emojis}
# Based on https://www.hvitfeldt.me/blog/real-emojis-in-ggplot2/
emoji_to_link <- function(emoji) {
    paste0("https://emojipedia.org/emoji/", emoji) %>%
    read_html() %>%
    html_nodes("tr td a") %>%
    .[1] %>%
    html_attr("href") %>%
...
tweets %>%
    ...
    slice_max(n, n = 10) %>%
    mutate(
        url   = map_chr(emoji, slowly(~emoji_to_link(.x), rate_delay(1))),
    ...
    ggplot(aes(reorder(label, n), n)) +
        ...
        labs(x = "Emoji", y = "Tweets",
             title = paste(params$hashtag, "emojis"),
             subtitle = "10 most commonly used emojis in original tweets")
```
